...
if __name__== "__main__": 
    ...
    #file = "Examples/0.2mm-cal/DataAvg.txt"
    #df = pd.read_csv(file)
    #fit_data_no_rot(df) 
    R=0.003
    L=0.002
    ...
    xx, yy, zz = np.meshgrid(x, y, z)
    np.shape(xx)
    Bx, By, Bz = Bxyz_func(xx, yy, zz, R, L, C)
    
    xx = eo.rearrange(xx, "x y z -> (x y z)")
    yy = eo.rearrange(yy, "x y z -> (x y z)")
    zz = eo.rearrange(zz, "x y z -> (x y z)")
    Bx = eo.rearrange(Bx, "x y z -> (x y z)")
    By = eo.rearrange(By, "x y z -> (x y z)")
    Bz = eo.rearrange(Bz, "x y z -> (x y z)")
    
    names = ['x', 'y', 'z', 'Mx', 'My', 'Mz']
    df_dict = {key: val for key, val in zip(names, [xx, yy, zz, Bx, By, Bz])}
