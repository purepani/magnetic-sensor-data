...
def print_time(t, axis):
    print(
        f"Estimate time remaining based on {axis} samples: {round(t/(60*60), 2)} hours, or {round(t/(60*60*24), 2)} days."
    )
...
sensor = Sensor(0x1E)
sensors = [
    Sensor(0x1D, i2c_dev=1),
    Sensor(0x1E, i2c_dev=5),
    Sensor(0x1E, i2c_dev=1),
    Sensor(0x1D, i2c_dev=5),
]
sensor_positions = np.array(
    [[0.0, 0.0, 0.0], [35.0, 0.0, 0.0], [0.0, 35.0, 0.0], [35.0, 35.0, 0.0]]
)
...
columns = ["Sensor Position"] + [
    x + f"{coordinate}" for x in column_categories for coordinate in ["x", "y", "z"]
]
...
try:
    ...
    for z in zz:
        ...
        for x in xx:
            ...
            for y in yy:
                ...
                cols = pd.MultiIndex.from_product(
                    [np.array(["position", "Magnetometer"]), np.array(["x", "y", "z"])],
                    names=["values", "axis"],
                )
                ind = pd.MultiIndex.from_product(
                    [range(len(sensors)), range(samples_per_measurement)],
                    names=["Sensor", "Sample"],
                )
                ...
                estimated_y = mean_times["y"].mean * (
                    total_y_samples - mean_times["y"].num
                )
        ...
        try:
            git_push(
                repo, f"Added the {z} slice to the {folder_name} test", folder_name
            )
        except:
...
github_repo.get_workflows()[0].create_dispatch(
    f"refs/heads/{branch_name}", inputs=input_payload
)
